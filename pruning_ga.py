...
class BlockPruningMask:
    ...
    def random_prune(self):
      """Fallback random pruning method"""
      n_prune = int(self.total_blocks * self.pruning_rate)
...
class Individual:
    ...
    def initialize_random_masks(self, model: nn.Module, importance_handler=None):
      """Initialize importance-aware pruning masks for prunable layers"""
      for name, module in model.named_modules():
        if self._is_prunable_layer(name, module):
            ...
            # Try to get importance scores for this layer
            importance_tensor = None
            if importance_handler and importance_handler.importance_dict:
                matched_key = importance_handler.match_layer_names(name, list(importance_handler.importance_dict.keys()))
                if matched_key:
                    importance_tensor = importance_handler.importance_dict[matched_key]
            ...
            # Use importance-aware pruning if available, otherwise random
            mask.importance_aware_prune(importance_tensor)
...
class GeneticPruner:
    ...
    def _prepare_dataloader(self):
      """Prepare evaluation dataloader with samples from BoolQ dataset"""
      # Load BoolQ dataset
      dataset = load_dataset('boolq', split='validation')
    
      def format_boolq_text(examples):
        """Format BoolQ examples into text for language modeling"""
        formatted_texts = []
        for i in range(len(examples['question'])):
            # Format: "Passage: {passage} Question: {question} Answer: {answer}"
            passage = examples['passage'][i]
            question = examples['question'][i] 
            answer = "Yes" if examples['answer'][i] else "No"
            
            formatted_text = f"Passage: {passage} Question: {question} Answer: {answer}"
            formatted_texts.append(formatted_text)
        
        return {'text': formatted_texts}
    
      # Format the dataset
      formatted_dataset = dataset.map(format_boolq_text, batched=True, 
    ...
    def initialize_population(self):
      """Initialize population with importance-aware pruning"""
